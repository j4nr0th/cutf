...
int main(void)
{
    ...
    for (unsigned i = 0; i < num_test_pairs; ++i)
    {
        ...
        cutf_state_t ctx = {0};
        auto const res = cutf_s8tos16(test_pairs[i].sz8, test_pairs[i].p8, sizeof(out) / sizeof(*out), &consumed, out,
                                      &written, &ctx);
    ...
    // Check that incorrect codepoints are rejected
    {
        size_t consumed, written;
        cutf_state_t ctx = {0};
        auto res = cutf_s8tos16(1, u8"\xBF", sizeof(out) / sizeof(*out), &consumed, out, &written, &ctx);
        TEST_ASSERT(res == CUTF_INVALID_INPUT);
        res = cutf_s8tos16(1, u8"\xFF", sizeof(out) / sizeof(*out), &consumed, out, &written, &ctx);
        TEST_ASSERT(res == CUTF_INVALID_INPUT);
    }
